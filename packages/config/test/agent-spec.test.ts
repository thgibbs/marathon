...
describe("parseAgentSpec (Tracks 12 + 14)", () => {
  ...
  it("chat grounding defaults on when a repo is set, and parses the explicit block (chat-repo.md)", () => {
    ...
    expect(
      parseAgentSpec({ name: "forge", instructions: "x", repo: "o/r", chat: { trusted_deployment: true } }).chat
        .trustedDeployment,
    ).toBe(true);
    expect(() => parseAgentSpec({ name: "forge", instructions: "x", chat: { trusted_deployment: "yes" } })).toThrow(
      /chat.trusted_deployment/,
    );
